...
btnKirim.addEventListener("click", async () => {
    ...
    if (!email) {
        ...
        notifBox.innerText = "Email harus diisi!";
        return;
    }

    try {
        const { error } = await client.auth.resetPasswordForEmail(email, {
            redirectTo: "https://website-game-tebak-gambar.vercel.app/UbahPassword.html"
        });

        if (error) throw error;

        notifBox.style.display = "flex";
        notifBox.innerText = "Link reset password sudah dikirim ke email!";
        notifBox.style.color = "green";

    } catch (err) {
        notifBox.style.display = "flex";
        notifBox.innerText = "Gagal: " + err.message;
        notifBox.style.color = "red";
    }
});
